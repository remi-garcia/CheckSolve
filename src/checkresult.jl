function _check_constraint(val::Float64, set::MOI.LessThan; kwargs...)::Float64
    return max(0.0, val - set.upper)
end
function _check_constraint(val::Float64, set::MOI.GreaterThan; kwargs...)::Float64
    return max(0.0, set.lower - val)
end
function _check_constraint(val::Float64, set::MOI.EqualTo; kwargs...)::Float64
    return abs(val - set.value)
end
function _check_constraint(val::Float64, set::MOI.Interval; kwargs...)::Float64
    return max(max(0.0, val - set.upper), max(0.0, set.lower - val))
end

function _check_constraint(con::ConstraintRef, set::Union{MOI.LessThan, MOI.GreaterThan, MOI.EqualTo, MOI.Interval}; round_int::Bool=true, kwargs...)::Float64
    if round_int
        return _check_constraint(value_int(con), set)
    end
    return _check_constraint(value(con), set)
end

function __check_constraint(con::ConstraintRef, set::MOI.Indicator; round_int::Bool=true, kwargs...)::Float64
    if round_int
        return _check_constraint(value_int(con)[2], set.set; kwargs...)
    end
    return _check_constraint(value(con)[2], set.set; kwargs...)
end

function _check_constraint(con::ConstraintRef, set::MOI.Indicator{MOI.ACTIVATE_ON_ONE}; kwargs...)::Float64
    if iszero(value_int(con)[1])
        return 0.0
    end
    return __check_constraint(con, set; kwargs...)
end

function _check_constraint(con::ConstraintRef, set::MOI.Indicator{MOI.ACTIVATE_ON_ZERO}; kwargs...)::Float64
...
function check_result(
    ...
    for i in 1:length(all_cons_ind_affexpr)
        ...
        if print_all_cst_check && !iszero(val_check_results[length(all_cons_affexpr)+i])
            println("Constraint: $(curr_constraint)")
            println("\tError: $(val_check_results[length(all_cons_affexpr)+i])")
            if print_var_values
